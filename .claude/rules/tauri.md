...
- `beforeDevCommand` / `beforeBuildCommand` 必须保证 `dimweave-bridge` 先被构建，否则 `.mcp.json` 注册出来的命令会指向不存在的二进制
...
- 每次做完 Tauri / daemon / Rust 链路审查后，必须同步更新 `docs/dimweave-audit-summary.md`；如果涉及 Claude / Codex / bridge 专项协议，还要同步更新对应 `docs/agents/*.md`
