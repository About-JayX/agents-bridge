...
> 目标：系统性对比 Claude Code 所有可用集成方式，为 Dimweave 找到比当前 `channel` 方案更优的接入路径。
>
> 2026-04-02 更新：Dimweave 当前执行路线已经明确选择 `--sdk-url` 作为 Claude 主链路。本文仍保留全量对比，用于解释为什么做这个选择，以及哪些风险仍需要宿主自己承担。
>
> **文档状态：** `Alternatives`
>
> **当前采用方案：** `--sdk-url` transport + MCP tools bridge
> **非当前方案：** Claude Agent SDK、CLI stdio stream-json、Channel transport、直接 Anthropic API
...
| Channel (旧主方案 / 历史对照) | ★★★ | ★★★ | ★★★ | claude.ai | 仅作历史对照 |
| 直接 Anthropic API | ★★★★★ | ★★★★★ | ★★★★★ | API key | 最终形态 |

**策略判断（文档级）:** 如果只看长期稳定性，Claude Agent SDK (`@anthropic-ai/claude-agent-sdk`) 仍然是最强的官方公共接口。  
**产品判断（Dimweave 当前）:** 为了保留 Rust 宿主、claude.ai 认证模型和更强的本地会话控制，当前实现路线选用 `--sdk-url`，不走 Agent SDK。
...
const q = query({
  ...
  options: {
    mcpServers: {
      "agentnexus": {
        command: "/path/to/dimweave-bridge",
...
**V2 对 Dimweave 的意义：**
- `send()` / `stream()` 直接对应 Dimweave 的 "路由消息到 Claude" + "接收 Claude 回复" 模型
...
**高度契合的部分：**
- Dimweave 的核心需求就是"协调多个 agent 协同工作"
- Agent Teams 的 task list + mailbox 模型和 Dimweave 的 task_graph + routing 非常对应
...
- **固定 lead** — 不能动态切换 lead，而 Dimweave 的 lead 可以是任何 provider
- **无 programmatic API** — 目前只通过 interactive CLI 控制，没有 SDK 接口

### 判断

**短期不适合替代 channel，但长期值得关注。** 如果 Agent Teams 稳定下来并获得 SDK 支持（能 programmatically spawn teammates、send messages、manage tasks），它可能成为 Dimweave Claude 侧的最佳选择。

---

## 方案三：Channel（旧主方案 / 历史对照）
...
## 补充：`--agent` flag 可以改善旧 channel 方案
...
---
name: lead
description: Dimweave lead coordinator
model: inherit
---

你是 Dimweave 多 agent 协作系统中的 lead 角色。
...
**对 Dimweave 的意义：** 当前 `provider/claude.rs` 扫描的就是这个路径。用 SDK `listSessions()` 可以直接替代自写的扫描逻辑。
...
- **可能的用途：** 如果 Dimweave 能注册为 remote-control 的 work source，可以完全避免 PTY/channel/subprocess 管理
...
**注意：** 这个 bridge 和 Dimweave 的 `bridge/` 不是同一个东西。Claude Code 的 bridge 是连 claude.ai 的远程协议，我们的 bridge 是 MCP channel sidecar。
